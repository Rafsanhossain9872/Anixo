    ...
    <div className="w-full max-w-[1500px] mx-auto px-4 md:px-8 mb-10 mt-6">
      <div className="bg-[#1a1217] border border-white/10 rounded-xl p-4 md:p-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-5">
        ...
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-discord-500/10 flex items-center justify-center shrink-0">
            <Share2 className="w-5 h-5 text-discord-500" />
          </div>
          <div>
            <h3 className="text-white font-medium text-[16px] md:text-[18px] leading-tight">{t('share.loveThisSite')}</h3>
            <p className="text-white/40 text-[13px] mt-0.5">{t('share.shareIt')}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {shareLinks.map((social) => (
            <button
              ...
              className={`${social.color} h-9 px-4 rounded flex items-center gap-2 hover:brightness-110 active:scale-95 transition-all shadow-sm`}
              aria-label={`Share on ${social.name}`}
            >
              <span className="text-white">{social.icon}</span>
              <span className="text-white font-medium text-[13px] hidden sm:block">
                {social.name}
              </span>
            </button>
          ))}
