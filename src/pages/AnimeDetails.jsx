...
export default function AnimeDetails() {
  ...
  if (!anime) {
    return (
      <div className="min-h-screen text-white flex flex-col pt-[52px]">
        <Navbar />
        <div className="flex-1 flex items-center justify-center">{t('details.notFound')}</div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row gap-6 md:gap-10">
              ...
              {isBookmarked && !addingAction ? (
                <div className="w-full flex items-center justify-between bg-white/[0.05] border border-white/10 rounded-[4px] px-3 py-2">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-[#aaa] uppercase tracking-wider font-bold">{t('details.inList')}</span>
          ...
          <div className="flex-1 text-center md:text-left mt-4 md:mt-24">
            ...
            <div className="text-[14px] text-[#ccc] leading-[1.8] max-w-[800px]">
              {anime.description ? (
                <div>{parse(DOMPurify.sanitize(anime.description))}</div>
              ) : (
                <p className="italic text-[#666]">{t('details.noSynopsis')}</p>
              )}
            </div>
          </div>
        </div>
